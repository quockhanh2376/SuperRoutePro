...
use super_route_pro_lib::repair_actions::{
    build_profile_cleanup_plan_for_target, validate_profile_cleanup_request,
};
use super_route_pro_lib::repair_protocol::ProfileCleanupRequest;
use super_route_pro_lib::repair_targets::{
    normalize_profile_root, validate_target_sid, RepairTargetUser,
};
...
fn repair_targets_normalize_profile_roots() {
    let normalized =
        normalize_profile_root(r" c:/Users/demo-user\ ").expect("profile root should normalize");
...
fn repair_targets_reject_non_profile_roots() {
    ...
    assert_eq!(
        normalize_profile_root(r"C:\Users\demo-user\AppData\Local"),
        None
    );
...
fn target_cleanup_resolves_profile_sensitive_paths_from_target_user() {
    ...
    assert!(plan
        .iter()
        .any(|path| path == r"C:\Users\demo-user\AppData\Local\Temp"));
    assert!(plan
        .iter()
        .any(|path| path
            == r"C:\Users\demo-user\AppData\Local\Microsoft\Edge\User Data\Default\Cache"));
    assert!(
